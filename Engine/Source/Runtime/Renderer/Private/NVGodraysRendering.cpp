...
/*=============================================================================
	NVGodraysRendering.cpp: NV Godrays rendering implementation.
=============================================================================*/
...
void FDeferredShadingSceneRenderer::NVGodraysBeginAccumulation(FRHICommandListImmediate& RHICmdList)
{
	check(Views.Num());
...
void FDeferredShadingSceneRenderer::NVGodraysRenderVolume(FRHICommandListImmediate& RHICmdList, const FLightSceneInfo* LightSceneInfo, const FProjectedShadowInfo* ShadowInfo)
{
	FSceneRenderTargets& SceneContext = FSceneRenderTargets::Get(RHICmdList);
...
void FDeferredShadingSceneRenderer::NVGodraysEndAccumulation(FRHICommandListImmediate& RHICmdList)
{
	RHICmdList.EndAccumulation();
}

void FDeferredShadingSceneRenderer::NVGodraysApplyLighting(FRHICommandListImmediate& RHICmdList)
{
	check(Views.Num());
	FSceneRenderTargets& SceneContext = FSceneRenderTargets::Get(RHICmdList);
	const FViewInfo& View = Views[0];
